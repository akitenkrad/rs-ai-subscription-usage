...
**`アクセストークンが失効しています`**．リフレッシュトークンが使える見込みがある一時的な失効です．stdout に案内を出して正常終了し（終了コード `0`），今回の取得を見送ります．JSONL と前回の `_limits.json` は変更しません．Claude Code を起動するとトークンが更新されます．リフレッシュトークンの失効時刻が不明な場合も，非空ならこの扱いです．

**`Claudeの認証情報の有効期限が切れています`**．アクセストークンが失効し，リフレッシュトークンも無い・空・失効済みのため使えません．stderr にエラーを出して終了します（終了コード `1`）．Claude Code を起動して `/login` し直してください．
